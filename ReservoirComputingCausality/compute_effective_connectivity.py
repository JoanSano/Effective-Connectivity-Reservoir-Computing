...
def process_single_subject(subject_file, opts, output_dir, json_file_config, format='svg', factor=10):
    ...
    name_subject = subject_file.split("/")[-1].split("_TS")[0] + '_Length-' + str(length) + '_Method-RCC'
    print("++++++++++++++++++++++++++++++++")
    ...
    for i, roi_i in enumerate(ROIs):
        for j in range(i if run_self_loops else i+1, len(ROIs)):
            ...
            x2y, y2x, _, _ = RCC_average(
                ...
                lags, I2N, N2N, split=split, skip=skip, 
                shuffle=False, axis=1, runs=runs, average=False
            )
            print("Done!")
            print("-------------------------------")
            ...
            surrogate_x2y, surrogate_y2x = surrogate_reservoirs(
                TS2analyse[i], TS2analyse[j], N_surrogates, 
            ...
            # RCC Scores
            print(f"Computing directionality for ROIs [{roi_i},{roi_j}]")
            evidence_xy, evidence_x2y, evidence_y2x, Score_xy, Score_x2y, Score_y2x = directionality_test(
                x2y, y2x, surrogate_x2y, surrogate_y2x, lags, 
            ...
            generate_report(
                ...
                plot=[opts.plot, format]
            )
            print("Done!")
            print("-------------------------------")
    
    # Build and save directed network
    print("Reconstructing directed network")
    process_subject_summary(output_dir, name_subject)
    print("Subject done!")
