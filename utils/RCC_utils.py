...
def reservoir_input2output(input, output, lag, I2N, N2N, split=75, skip=20, axis=0):
    """
    ...
    input_train, output_train, input_test, output_test = split_train_test_reshape(input_data, output_data, split, axis=axis)
...
def RCC_input2output(input, output, lags, I2N, N2N, split=75, skip=20, axis=0):
    """
    TODO: Add description
    """
    ...
    for lag in lags:
        # x(t) predicts y(t+t*)
        rho, results = reservoir_input2output(input, output, lag, I2N, N2N, split=split, skip=skip, axis=axis)
        results_i2o.append(results)
        correlations_i2o.append(rho)

        # y(t) predicts x(t+t*)
        rho, results = reservoir_input2output(output, input, lag, I2N, N2N, split=split, skip=skip, axis=axis)
        results_o2i.append(results)
        correlations_o2i.append(rho)
    return correlations_i2o, correlations_o2i, results_i2o, results_o2i

def RCC_statistics(x, y, lags, runs, I2N, N2N, split=75, skip=20, axis=0):
    """
    TODO: Add description
    """
    ...
    for run in range(runs):
        # Single RCC run
        correlations_x2y[run,:], correlations_y2x[run,:], r_x2y, r_y2x = RCC_input2output(x, y, lags, I2N, N2N, split=split, skip=skip, axis=axis)
