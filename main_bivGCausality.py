if __name__ == '__main__':
    ## Relative imports
    from methods.estimators.granger_based import bivariate_GC
    ...
    for sim_num in [1,7,15,19]: # Simulation 28 should have a minimum lag of -20, otherwise it will raise an error
        for L in [70,75,80,85,90,95,100]:
            args = {
                ...
                "r_folder": f"Results_Netsim-Dataset_Sim-{sim_num}_Method-bivGC/Results_Netsim-Dataset_Sim-{sim_num}_Length-{L}",
                "num_jobs": 4,
                "length": L,
                "subjects": ["-1"],
                "rois": [-1],
